...
class World(object):
    ...
    def __init__(self, worlddir):
        ...
        for root, dirs, files in os.walk(self.worlddir, followlinks=True):
            ...
            mcas = [x for x in files if x.endswith(".3dr")]
    ...
    def find_true_spawn(self):
        ...
        if not regionset:
            return None
        try:
            chunk = regionset.get_chunk(chunkX, chunkY, chunkZ)
        except ChunkDoesntExist:
        ...
        #         if section['Blocks'][inChunkX, inChunkY, inChunkZ] == 0:
        #             return spawnX, spawnY, spawnZ
...
class RegionSet(object):
    ...
    def get_chunk(self, x, y, z):
        ...
        * For each chunk section:
        ...
        # Tile Entities:
        color_array = 2 ** 30 * numpy.ones((16,16,16), dtype=numpy.uint32)
        if "TileEntities" in chunk_data:
            for tile_entity in chunk_data["TileEntities"]:
                if "stencilsEast" in tile_entity:
                    cx = tile_entity["x"]
                    cy = tile_entity["y"]
                    cz = tile_entity["z"]
        ...
        for section in chunk_data['Sections']:
            ...
            try:
                ...
                (section['Blocks'], section['Data'], section['Colors']) = (blocks, data, color_array)
    ...
    def get_chunk_mtime(self, x, y, z):
        ...
        try:
            ...
            logging.warning("Ignoring request for chunk %s,%s; region %s,%s seems to be corrupt",
                    x,z, x//32,z//32)
            return None
        if data.chunk_exists(x,y,z):
            d =  data.get_chunk_timestamp(x,y,z)
            if d is None:
                print("no chunk data in file {},{},{}".format(x//16,y//16,z//16))
            return data.get_chunk_timestamp(x,y,z)
        return None

    def _get_region_path(self, chunkX, chunkY, chunkZ):
...
class RegionSetWrapper(object):
    ...
    def get_chunk_mtime(self, x, y, z):
        return self._r.get_chunk_mtime(x,y,z)
...
class CroppedRegionSet(RegionSetWrapper):
    ...
    def get_chunk(self,x,y,z):
        if (
            ...
            return super(CroppedRegionSet, self).get_chunk(x,y,z)
        else:
