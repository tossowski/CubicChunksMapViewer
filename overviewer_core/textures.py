...
modpath = "/mnt/c/Users/tosso/Desktop/AT/instances/MysticsCubicChunksModpack/mods/"
...
class Textures(object):
    ...
    def find_file(self, filename, mode="rb", verbose=False, modname=None):
        ...
        * The overviewer_core/data/textures dir
        ...
        while(available_versions):
            ...
            if modname == None:
                ...
                jarpath = os.path.join(modpath, modname + ".jar")
    ...
    def load_image(self, filename, modname=None):
        ...
        try:
            fileobj = self.find_file(filename, verbose=logging.getLogger().isEnabledFor(logging.DEBUG), modname=modname)
...
def block(blockid=[], top_image=None, side_image=None, **kwargs):
    ...
    def inner_block(self, unused_id, unused_data):
        if "modname" in kwargs:
            return self.build_block(self.load_image_texture(top_image, kwargs["modname"]), self.load_image_texture(side_image, kwargs["modname"]))
        else:
