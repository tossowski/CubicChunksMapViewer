...
class MCRFileReader(object):
    ...
    _location_table_format = struct.Struct(">4096I")
    _timestamp_table_format = struct.Struct(">4096i")
    _chunk_header_format = struct.Struct(">I")
    ...
    def __init__(self, fileobj):
        ...
        location_data = self._file.read(4096 * 4)
        if not len(location_data) == 4096 * 4:
            raise CorruptRegionError("invalid location table")
        # read in the timestamp table
        timestamp_data = self._file.read(4096 * 4)
        if len(timestamp_data) == 4096 * 4:
            self._timestamps = self._timestamp_table_format.unpack(timestamp_data)
        else:
            self._timestamps = [0] * 4096
    ...
    def get_chunks(self):
        ...
        for x in range(16):
            for y in range(16):
                for z in range(16):
                    if self._locations[int(x * 256 + y * 16 + z)] >> 8 != 0:
                        yield (x, y, z)

    def get_chunk_timestamp(self, x, y, z):
        ...
        y = y % 16
        z = z % 16
        return self._timestamps[int(x * 256 + y * 16 + z)]
    ...
    def chunk_exists(self, x, y, z):
        ...
        y = y % 16
        z = z % 16
        return self._locations[int(x * 256 + y * 16 + z)] >> 8 > 0
    ...
    def load_chunk(self, x, y, z):
        ...
        data_length = self._chunk_header_format.unpack(header)[0]
