...
static inline void load_chunk_section(ChunkData* dest, int32_t i, PyObject* section) {
    ...
    dest->sections[i].colors = (PyArrayObject*)PyDict_GetItemString(section, "Colors");
    ...
    Py_INCREF(dest->sections[i].colors);
}
        ...
        for (j = 0; j < 3; j++) {
            if (state->chunks[i][j].loaded) {
                ...
                for (k = 0; k < SECTIONS_PER_CHUNK; k++) {
                    ...
                    Py_XDECREF(state->chunks[i][j].sections[k].colors);
                }
